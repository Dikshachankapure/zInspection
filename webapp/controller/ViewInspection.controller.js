...
], function (Controller, History, Filter, FilterOperator, JSONModel, formatter, Item, MobileLibrary, Uploader, ListItem) {
	"use strict";

/* Maurice added */
/*var ListMode = MobileLibrary.ListMode;

	var CustomUploader = Uploader.extend("poc.zinspection.UploadSetCustomUploader.CustomUploader", {
		metadata: {}
	});
...
/*up to here */
	...
	return Controller.extend("poc.zinspection.controller.ExecuteInspection", {
		...
		onInit: function () {
			...
			// Maurice added here
/*			var sPath = sap.ui.require.toUrl("jsonData/Items.json"),
			...
			if (tblLocations.getItems().length > 0) {
				...
				if (Valresult == true) {
					...
					if (this.countQuestions === 1) {
						...
						this.percentage = 100 / parseInt(this.countQuestions);
						if (this.percentage > 0) {
							this.progressvalue = parseInt(this.progressvalue) + parseInt(this.percentage);
						}
					}

					indicator.setPercentValue(this.progressvalue);
					indicator.setDisplayValue(this.progressvalue + "%");
				...
				if (this.countQuestions === 1) {
					...
					this.percentage = 100 / parseInt(this.countQuestions);
					if (this.percentage > 0) {
						this.progressvalue = parseInt(this.percentage) * parseInt(this.tblselectedvalues);
					}
				}
